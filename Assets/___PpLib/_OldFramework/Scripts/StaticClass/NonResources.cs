...
namespace SR
{
    ...
    public static class NonResources
    {
        ...
        /// <summary>
        /// ファイルのパス("Assets/~/~~.asset")と型を設定し、Objectを読み込む。存在しない場合はNullを返す
        /// </summary>
        ...
        /// <summary>
        /// ファイルのパス("Assets/~/~~.asset")を設定し、Objectを読み込む。存在しない場合はNullを返す
        /// </summary>
        ...
        /// <summary>
        /// 同階層のobjectを読み込む。存在しない場合は空のListを返す
        /// 
        /// ファイルパス("Assets/~/~~.asset")
        /// </summary>
        public static List<T> LoadAll<T>(Object obj) where T : Object
        {
            var p = UnityEditor.AssetDatabase.GetAssetPath(obj);
            var path = p.Substring(0, p.LastIndexOf("/") + 1);
            SLog.Check.Info(path);
